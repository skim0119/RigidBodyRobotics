"""Unit tests for elastica_rigid.body._se2_equations."""

import numpy as np

from elastica_rigid.body._se2_equations import (
...
class TestUpdateRigidSO2KinematicState:
    ...
    def test_position_update(self) -> None:
        ...
        np.testing.assert_allclose(position[:, 0], [2.0, 1.0])
        np.testing.assert_allclose(direction[:, 0], [1.0, 0.0])  # no rotation
    ...
    def test_direction_rotation_90_deg(self) -> None:
        ...
        np.testing.assert_allclose(direction[:, 0], [0.0, 1.0], atol=1e-14)
...
class TestUpdateRigidSO2DynamicState:
    ...
    def test_velocity_and_omega_update(self) -> None:
        ...
        np.testing.assert_allclose(velocity[:, 0], [2.0, 0.0])  # 1 + 0.5*2
...
class TestUpdateAccelerations:
    ...
    def test_acceleration_equals_force_over_mass(self) -> None:
        ...
        np.testing.assert_allclose(acceleration[:, 0], [2.0, 1.0])
...
class TestZeroedOutExternalForcesAndTorques:
    ...
    def test_arrays_zeroed(self) -> None:
        ...
        external_forces = np.array([[1.0], [3.0]])
        external_torques = np.array([1.0])
