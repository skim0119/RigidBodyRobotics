...
class TestComputeWheelForcesToExternal:
    ...
    def test_forward_direction_symmetric_wheels(self) -> None:
        ...
        np.testing.assert_allclose(external_torques, [0.0])
    ...
    def test_rotated_direction(self) -> None:
        ...
        # (track_width/2)*(-left_x+right_x) = 0.1 * (-1) = -0.1
        np.testing.assert_allclose(external_torques, [-0.1])
    ...
    def test_adds_to_existing(self) -> None:
        ...
        np.testing.assert_allclose(external_torques, [0.4])  # 0.5 + (-0.1)
